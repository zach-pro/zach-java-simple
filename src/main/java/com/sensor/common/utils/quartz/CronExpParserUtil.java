...
import com.sensor.common.constant.WeekEnum;
...
{
    protected static final String[] CRON_TIME_CN = new String[]
            {
                    "秒", "分钟", "小时", "天", "月", "周", "年"
            };
    private static final Integer HOURS = 24;
    private static final Integer TIMESCALE = 60;
    /**
    ...
    {
        String[] cronTimeArr = CRON_TIME_CN;
        if(cronExp == null || cronExp.length() < 1)
        {
            return "cron表达式为空";
        }
        if (!CronUtils.isValid(cronExp))
        {
            return "cron表达式无效";
        }
        ...
        if(tmpCorns.length == 6 || tmpCorns.length == 7)
        {
            ...
            if(!months.equals("*") && !months.equals("?"))
            {
                if(months.contains("/"))
                {
                    sBuffer.append("从").append(months.split("/")[0]).append("号开始").append(",每").append(months.split("/")[1]).append(cronTimeArr[4]);
                }
                else
                {
            ...
            if(!dayofWeek.equals("*") && !dayofWeek.equals("?"))
            {
                ...
                else if(dayofWeek.contains("L") && dayofWeek.length() > NumberUtils.INTEGER_ONE)
                { // 1L-7L
            ...
            if(!days.equals("?") && !days.equals("*"))
            {
                if(days.contains("/"))
                {
                    sBuffer.append("每周从第").append(days.split("/")[0]).append("天开始").append(",每").append(days.split("/")[1]).append(cronTimeArr[3]);
                }
            ...
            if(!minutes.equals("*"))
            {
                ...
                else if(minutes.contains("-"))
                {
                    ...
                    sBuffer.append(splitMinute[0]).append(cronTimeArr[1]).append("到").append(splitMinute[1]).append(cronTimeArr[1]).append("每分钟");
                }
                else
                {
    ...
    {
        StringBuilder sBufferTemp = new StringBuilder();
        ...
        sBufferTemp.append("从").append(startNum).append(timeUnit).append("开始").append("到").append(endStr).append(timeUnit).append("范围内").append(",每隔").append(gapNum).append(timeUnit);
        return sBufferTemp.toString();
    }

    public static void main(String[] args) {
        log.info(CronExpParserUtil.translateToChinese("0 15 10 ? * 2-6"));
    }
}